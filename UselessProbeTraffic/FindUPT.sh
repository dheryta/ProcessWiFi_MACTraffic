...
codePath="/home/dherytaj/Scripts/CausalAnalysis/UselessProbeTraffic/"
ssid=$3
clients=$4
...
gcc $codePath/ParsePCAPForSimilarityIndex.c -o $codePath/ParsePCAPForSimilarityIndex.o
...
$codePath/ParsePCAPForSimilarityIndex.o /tmp/mac_frames.csv $mac $ssid > $folder/$mac.csv
...
if [ -f $folder/AllMAC-AllProbeDetails.csv ]; then
rm $folder/AllMAC-AllProbeDetails.csv
fi

for file in `ls -v $folder/*.csv`
do
	tail -1 $file >> $folder/AllMAC-AllProbeDetails.csv
done
